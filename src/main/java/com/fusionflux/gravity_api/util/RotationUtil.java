...
public abstract class RotationUtil {
    ...
    static {
        for (Direction gravityDirection : Direction.values()) {
            ...
            for (Direction direction : Direction.values()) {
                ...
                DIR_WORLD_TO_PLAYER[gravityDirection.getId()][direction.getId()] = Direction.fromVector(BlockPos.fromPosition(directionVector));
            }
        }
    }
    ...
    static {
        for (Direction gravityDirection : Direction.values()) {
            ...
            for (Direction direction : Direction.values()) {
                ...
                DIR_PLAYER_TO_WORLD[gravityDirection.getId()][direction.getId()] = Direction.fromVector(BlockPos.fromPosition(directionVector));
            }
        }
    }
    ...
    public static Quaternionf getRotationBetween(Direction d1, Direction d2) {
        if (d1.getOpposite() == d2) {
            return Axis.Z_NEGATIVE.rotationDegrees(180.0F);
        } else {
            return QuaternionUtil.getRotationBetween(d1.getUnitVector(), d2.getUnitVector());
        }
    }
