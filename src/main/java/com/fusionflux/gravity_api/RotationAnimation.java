...
public class RotationAnimation {
    ...
    public void applyRotationAnimation(Direction newGravity, Direction prevGravity, long durationTimeMs, Entity entity, long timeMs, boolean rotateView) {
        ...
        Quaternionf currentAnimatedCameraRotation = new Quaternionf(oldViewRotation).mul(currentAnimatedGravityRotation);
        ...
        startGravityRotation = newViewRotation.conjugate().mul(currentAnimatedCameraRotation);
    ...
    private Vec3d getNewLookingDirection(
        ...
        Quaternionf deltaRotation = QuaternionUtil.getRotationBetween(
                prevGravity.getUnitVector(),
                newGravity.getUnitVector()
        );

        Vector3f lookingDirection = new Vector3f((float) oldLookingDirection.getX(), (float) oldLookingDirection.getY(), (float) oldLookingDirection.getZ());
    ...
    public Quaternionf getCurrentGravityRotation(Direction currentGravity, long timeMs) {
        ...
        return new Quaternionf(startGravityRotation).slerp(endGravityRotation, mapProgress((float) delta));
    }

    public boolean isNotInAnimation() {
        return !inAnimation;
    }
